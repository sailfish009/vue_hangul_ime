...
var unicode_NFD_hangeul_sidedot = [0x302E,0x302F]; // 옛한글에서 성조를 나타내는 방점
...
function initialize_options() {
	...
	if(typeof enable_sign_ext != 'undefined') default_enable_sign_ext = option.enable_sign_ext;
	if(typeof force_normal_typing != 'undefined') default_force_normal_typing = option.force_normal_typing;
	if(typeof phonemic_writing != 'undefined') default_phonemic_writing = option.phonemic_writing;
	if(typeof phonemic_writing_in_single_phoneme != 'undefined') default_phonemic_writing_in_single_phoneme = option.phonemic_writing_in_single_phoneme;
	if(typeof phonemic_writing_in_halfwidth_letter != 'undefined') default_phonemic_writing_in_halfwidth_letter = option.phonemic_writing_in_halfwidth_letter;
	if(typeof phonemic_writing_initial_ieung_ellipsis != 'undefined') default_phonemic_writing_initial_ieung_ellipsis = option.phonemic_writing_initial_ieung_ellipsis;
	if(typeof phonemic_writing_adding_space_every_syllable_end != 'undefined') default_phonemic_writing_adding_space_every_syllable_end = option.phonemic_writing_adding_space_every_syllable_end;
	if(typeof phonemic_writing_directly != 'undefined') default_phonemic_writing_directly = option.phonemic_writing_directly;
	if(typeof square_layout != 'undefined') default_square_layout = option.square_layout;
...
function NFC_Sin3_preprocess(f,e,key) { // 요즘한글 신세벌식 자판 처리
	...
	// 바꾼꼴 신세벌식 배열이면
	if(!with_shift_key(key) && unicode_ga.indexOf(Sin3_layout[64])>=0) transform=1;
	else if(with_shift_key(key) && unicode_ga.indexOf(Sin3_layout[shift_table[64]])>=0) transform=1;
	...
	if(c1<31 && !with_shift_key(key) && !ohiQ[0] && !ohiQ[3] && ohiQ[6] && !ohiQ[7]) {
	...
	else if(transform && c1<31 && !ohiQ[0] && !ohiQ[3] && ohiQ[6] && !ohiQ[7]) {
	// 받침을 윗글 자리에 두는 바꾼꼴 신세벌식 자판의 두번째 들어온 받침 처리
		...
		if(i) {
			i=convert_into_ohi_hangeul_phoneme(i);
			ohiQ[7]=i-ohiQ[6];
			ohiInsert(f,0,ohiQ);
			return -1;
		}
...
function NFD_Sin3_preprocess(f,e,key) { // 첫가끝 방식으로 조합하는 신세벌식 한글 처리 (옛한글)
	...
	if(option.enable_Sin3_diphthong_key && !with_shift_key(key) && NFD_stack.phoneme.length && unicode_cheos.indexOf(NFD_stack.phoneme[0])>=0 && unicode_cheos.indexOf(c1)>=0 && unicode_ga.indexOf(c2)>=0) {
...
function show_keyboard_layout(type) {
	...
	for(i=0, k=-1; ue[i]; i++) {
		...
		for(j=0; ue[i][j]; j++) {
			...
			var col = document.body.appendChild(row,'div',tdclass,tdid,'','36px','0 0 0 0');
			...
			var up = document.body.appendChild(col,'div','up','up'+k);
			document.body.appendChild(up,'div','ue','ue'+k,ue[i][j]);
			if(uh[i]) {
				if(uh[i][j]) {
				...
				document.body.appendChild(up,'div','uh','uh'+k,uh[i][j]);
			}
			if(de[i][j]) {
				var down = document.body.appendChild(col,'div','down','down'+k);
				...
				document.body.appendChild(down,'div','de','de'+k,de[i][j]);
				if(dh[i] && (!dh[i][j] || dh[i][j]==de[i][j])) dh[i][j]=' ';
				if(dh[i] && dh[i][j]) document.body.appendChild(down,'div','dh','dh'+k,dh[i][j]);
			}
		}
	}
...
function ohi_code_tables() {
	...
	var ohi_hangeul_phoneme = ohi_cheos.concat(ohi_ga,ohi_ggeut);
	ohi_hotbadchim = [/*ㄱ*/1,/*ㄴ*/4,/*ㄷ*/7,/*ㄹ*/9,/*ㅁ*/17,/*ㅂ*/18,/*ㅅ*/21,/*ㅇ*/23,/*ㅈ*/24,/*ㅊ*/26,/*ㅋ*/27,/*ㅌ*/28,/*ㅍ*/29,/*ㅎ*/30];
	var unicode_modern_hotbatchim = [/*ㄱ*/0x11A8,/*ㄴ*/0x11AB,/*ㄷ*/0x11AE,/*ㄹ*/0x11AF,/*ㅁ*/0x11B7,/*ㅂ*/0x11B8,/*ㅅ*/0x11BA,/*ㅇ*/0x11BC,/*ㅈ*/0x11BD,/*ㅊ*/0x11BE,/*ㅋ*/0x11BF,/*ㅌ*/0x11C0,/*ㅍ*/0x11C1,/*ㅎ*/0x11C2];
	...
	var halfwidth_hangeul_phoneme = halfwidth_cheos.concat(halfwidth_ga, halfwidth_ggeut);
	...
	var unicode_ggeut_to_cheos = [0x1100,0x1101,0,0x1102,0,0,0x1103,0x1105,0,0,0,0,0,0,0,0x1106,0x1107,0,0x1109,0x110A,0x110B,0x110C,0x110E,0x110F,0x1110,0x1111,0x1112,
	...
	var unicode_cheos_to_ggeut = [0x11A8,0x11A9,0x11AB,0x11AE,0xD7CD,0x11AF,0x11B7,0x11B8,0xD7E6,0x11BA,0x11BB,0x11BC,0x11BD,0xD7F9,0x11BE,0x11BF,0x11C0,0x11C1,0x11C2,
	...
	var unicode_NFD_hangeul_filler = [0x115F,0x1160]; // 첫가끝 조합형 첫소리·가운뎃소리 채움 문자
	var unicode_NFD_hangeul_code = unicode_NFD_hangeul_phoneme.concat(unicode_NFD_hangeul_filler); 	// 첫가끝 조합형에서 한글 낱내를 나타내는 데에 쓰이는 부호
	// var unicode_NFD_hangeul_sidedot = [0x302E,0x302F]; // 옛한글에서 성조를 나타내는 방점
	...
	var unicode_non_combined_cheos = [0x1100,0x1102,0x1103,0x1105,0x1106,0x1107,0x1109,0x110B,0x110C,0x110E,0x110F,0x1110,0x1111,0x1112];
	var unicode_non_combined_ga = [0x1161,0x1162,0x1164,0x1165,0x1166,0x1168,0x1169,0x1172,0x1173,0x1175,0x119E];
	var unicode_non_combined_ggeut = [0x11A8,0x11AB,0x11AE,0x11AF,0x11B7,0x11B8,0x11BA,0x11BC,0x11BD,0x11BE,0x11BF,0x11C0,0x11C1,0x11C2];
	var unicode_non_combined_phoneme = unicode_non_combined_cheos.concat(unicode_non_combined_ga, unicode_non_combined_ggeut);
...
export {ohi_code_tables, browser_detect, ohiStart, url_query, ohiChange_between_same_type}
